...
class LLMAdapterFactory:
    ...
    def create_cloud_adapter(
        ...
        Args:
            provider: Nombre del provider (openai, anthropic, google, groq, minimax, deepseek, lmstudio)
            model: Modelo especifico (None = default del provider)
            api_key: API key (None = .env, no usado para lmstudio)
        ...
        from src.infrastructure.adapters.lmstudio_llm_adapter import LMStudioLLMAdapter

        if provider == "lmstudio":
            adapter = LMStudioLLMAdapter(model=model)
            logger.debug(f"Created LMStudioLLMAdapter for model={model}")
            return adapter

...
def _get_local_adapter() -> "LLMPort":
    """Get local adapter - tries LM Studio first, then llama.cpp."""
    import os
    ...
    try:
        ...
        with httpx.Client(base_url=lm_studio_url, timeout=2.0) as client:
            response = client.get("/api/v1/models")
            response.raise_for_status()
        from src.infrastructure.adapters.lmstudio_llm_adapter import LMStudioLLMAdapter
        adapter = LMStudioLLMAdapter(base_url=lm_studio_url)
        return adapter
    except Exception:
        pass
    ...
    if not Path(model_path).exists():
        raise RuntimeError(
            ...
            f"Or make sure LM Studio is running at {lm_studio_url}"
        )

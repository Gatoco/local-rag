...
class DependencyValidator:
    ...
    def validate_python_version(self, min_version: tuple[int, int] = (3, 12),
                                 max_version: tuple[int, int] = (3, 13)) -> bool:
    ...
    def validate_all(self, strict: bool = False) -> bool:
        ...
        for package, info in self.OPTIONAL_PACKAGES.items():
            if not self.validate_package(package):
                if strict:
                    self.warnings.append(f"Opcional recomendado: {info['name']} ({info['feature']})")
